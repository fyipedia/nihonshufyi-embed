/* nihonshufyi-embed v1.1.2 | MIT | https://widget.nihonshufyi.com */
...
function kvRows(rows) {
  ...
  const items = rows.filter((r) => r.value).map(
    (r) => `
      <div class="drinkfyi-kv-row">
        <span class="drinkfyi-kv-label">${esc(r.label)}</span>
        <span class="drinkfyi-kv-value">${esc(r.value)}</span>
      </div>`
  ).join("");
  return items ? `<div class="drinkfyi-kv-rows">${items}</div>` : "";
}
...
// src/cards/nihonshu-card.ts
function polishingCircle(ratio) {
  if (ratio <= 0 || ratio > 100) return "";
  const innerSize = Math.max(20, Math.round(ratio * 0.7));
  return `
    <div class="drinkfyi-polish">
      <div class="drinkfyi-polish-label">Rice Polishing Ratio</div>
      <div class="drinkfyi-polish-outer">
        <div class="drinkfyi-polish-inner" style="width:${innerSize}px;height:${innerSize}px;">
          ${ratio}%
        </div>
      </div>
      <div class="drinkfyi-polish-note">${ratio}% of rice grain remaining</div>
    </div>`;
}
function renderNihonshuCard(container, data, config, _lang) {
  var _a, _b, _c, _d, _e, _f;
  const name = String((_a = data.name) != null ? _a : "");
  const japaneseName = String((_b = data.japanese_name) != null ? _b : "");
  const brewery = String((_c = data.brewery_name) != null ? _c : "");
  const grade = String((_d = data.grade_name) != null ? _d : "");
  const rice = String((_e = data.rice_name) != null ? _e : "");
  const polishing = data.polishing_ratio != null ? parseFloat(String(data.polishing_ratio)) : 0;
  const abv = data.abv ? `${data.abv}%` : "";
  const smv = data.smv != null ? String(data.smv) : "";
  const acidity = data.acidity != null ? String(data.acidity) : "";
  const amino = data.amino_acid != null ? String(data.amino_acid) : "";
  const url = String((_f = data.url) != null ? _f : `https://${config.domain}/${config.entitySlug}/`);
  const title = japaneseName || name;
  const subtitle = japaneseName ? name : "";
  const stats = [
    abv ? { value: abv, label: "ABV" } : null,
    smv ? { value: smv, label: "SMV" } : null,
    acidity ? { value: acidity, label: "Acidity" } : null,
    amino ? { value: amino, label: "Amino" } : null
  ].filter((s) => s !== null);
  const kvData = [
  ...
  container.innerHTML = [
    ...
    polishing > 0 ? polishingCircle(polishing) : "",
    statsRow(stats),
    kvRows(kvData),
...
// src/_entry_nihonshufyi.ts
setSiteCardRenderer(renderNihonshuCard);
...
(function bootstrap() {
  const config = '{"site":"nihonshufyi","name":"NihonshuFYI","domain":"nihonshufyi.com","accent":"#2563EB","attribute":"data-nihonshufyi","apiBase":"https://nihonshufyi.com/api/v1/","searchPath":"/search/","entityName":"Sake","entitySlug":"sake"}';
...
function makeWidgetElement(widgetType, initFn, domainAttrs) {
  ...
  return class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._syncDataAttrs();
      initFn(this, '{"site":"nihonshufyi","name":"NihonshuFYI","domain":"nihonshufyi.com","accent":"#2563EB","attribute":"data-nihonshufyi","apiBase":"https://nihonshufyi.com/api/v1/","searchPath":"/search/","entityName":"Sake","entitySlug":"sake"}');
    }
    ...
    _syncDataAttrs() {
      const attrKey = '{"site":"nihonshufyi","name":"NihonshuFYI","domain":"nihonshufyi.com","accent":"#2563EB","attribute":"data-nihonshufyi","apiBase":"https://nihonshufyi.com/api/v1/","searchPath":"/search/","entityName":"Sake","entitySlug":"sake"}'.attribute.replace("data-", "");
...
(function registerElements() {
  ...
  const site = '{"site":"nihonshufyi","name":"NihonshuFYI","domain":"nihonshufyi.com","accent":"#2563EB","attribute":"data-nihonshufyi","apiBase":"https://nihonshufyi.com/api/v1/","searchPath":"/search/","entityName":"Sake","entitySlug":"sake"}'.site;
  const defs = [
